...
On peut installer mysql avec le script issu des EmonScripts :

https://github.com/openenergymonitor/EmonScripts/blob/master/install/mysql.sh
...
*Nota pour info :*
pour créer un utilisateur avec des privilèges particuliers sur le port série : `sudo useradd -M -r -G dialout,tty -c "emonHub user" emonhub`. Si l'utilisateur est existant : `sudo usermod -a -G dialout,tty alexandrecuer`
